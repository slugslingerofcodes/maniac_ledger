...
`/search`, `/library`, `/upcoming`, `/recommendations`, and `/profile` live under `src/app/(app)/` and share `src/app/(app)/layout.tsx`, which renders the sticky `<AppNav/>` (avatar dropdown + mobile Sheet) plus the mobile `<BottomTabBar/>` (hidden at `md+`). Routes **outside** the group (`/`, `/anime/[id]`, the auth pages) still render the older `<SiteHeader/>`. This is a half-finished migration — both navs exist and **share the same `NAV_ITEMS` set, so a new authed route must be added to both** (`AppNav` *and* `SiteHeader`) or it's unreachable from one of them; prefer the `(app)` group for new authenticated pages.
...
- `generateRecommendations` calls **Gemini** (`@google/generative-ai`, `gemini-1.5-flash`, requires `GEMINI_API_KEY`), resolves each pick to a real `mal_id`/poster via Jikan, and denormalizes title/poster/score onto the row so the page needs no re-fetch.
