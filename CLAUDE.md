...
- **RLS is the *security* boundary, but no longer the *scoping* one for `user_progress`.** Migration `0015` added an **additive** SELECT policy ("public profiles progress readable"), and Postgres ORs permissive policies together — so an unfiltered read of `user_progress` returns your rows **plus every public profile's**. Every "my rows" query must therefore pass `eq("user_id", user.id)` explicitly (embedded reads use `eq("user_progress.user_id", …)`). Omitting it is the bug that put duplicate anime — with strangers' watch statuses — in the library grid, and it also breaks `maybeSingle()` reads, which error on multiple matches. `episode_progress` still has owner-only policies, so it remains RLS-scoped. Catalog tables (`anime`, `episodes`) are readable by, and insertable by, any authenticated user ("catalog contributions").
